...
class StationLoginScreen extends StatefulWidget {
  StationLoginScreen({Key? key}) : super(key: key);

  @override
  _StationLoginScreen createState() => _StationLoginScreen();
}

class _StationLoginScreen extends State<StationLoginScreen> {
  ...
  Future<void> _handleSignIn(BuildContext context) async {
    try {
      ...
      if (_formKey.currentState?.validate() ?? false) {
        ...
        if (stationId.isNotEmpty && password.isNotEmpty) {
          // Call the loginUser method from FirestoreService
          await _firestoreService.loginStation(
              stationId: stationId, password: password);

          // Navigate to the dashboard screen upon successful login
          Navigator.pushNamed(context, AppRoutes.stationDashboardScreen);
        } else {
